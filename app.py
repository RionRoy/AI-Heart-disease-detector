...
# --- 2.5 ECG CLONE DATA PARSER ---
@st.cache_data
def load_ecg_clone_data():
    """Parses Answer_Key.txt to map diseases to sample patient files."""
    answer_key_path = os.path.join(os.path.dirname(__file__), "ECG FOR TESTING", "Answer_Key.txt")
    clone_data_map = {}
    ...
    if os.path.exists(answer_key_path):
        ...
        for line in lines:
            ...
            if len(parts) == 2:
                ...
                try:
                    class_idx = int(parts[1].strip())
                    if class_idx < len(CLASS_NAMES_RAW):
                        class_code = CLASS_NAMES_RAW[class_idx]
                        full_name = DIAGNOSIS_MAP.get(class_code, class_code)
                        
                        # Store the first patient file we find for each disease
                        if full_name not in clone_data_map:
                            clone_data_map[full_name] = filename
                except ValueError:
                    continue
                    
    return clone_data_map
...
"""
        ...
        # 3. ECG CLONE (Disease Simulator)
        st.markdown("---")
        st.markdown("### 🧬 ECG CLONE: Disease Simulator")
        st.markdown("Visualize typical ECG patterns for related cardiac conditions.")
        
        clone_map = load_ecg_clone_data()
        if clone_map:
            # Sort disease names alphabetically for the dropdown
            available_diseases = sorted(list(clone_map.keys()))
            
            # Allow user to select a disease to simulate
            selected_disease = st.selectbox("Select Pathology to Simulate:", available_diseases)
            
            if selected_disease:
                sample_file = clone_map[selected_disease]
                sample_path = os.path.join(os.path.dirname(__file__), "ECG FOR TESTING", sample_file)
                
                if os.path.exists(sample_path):
                    st.markdown(f"#### Simulated Trace: {selected_disease}")
                    ...
                    # Display the simulated trace in a distinct color (e.g., orange/yellow)
                    st.line_chart(sample_data[0,:,0], height=200, color="#FFA500")
                else:
